...
# 이 무결선 테스트는 모든 DAG파일이 들어있는 폴더를 대상으로 하며, *.py파일을 반복적으로 탐색한다.
import glob
...
def test_json_to_csv_operator(tmp_path: Path): # tmp_path고정으로 사용
    ...
    assert result == input_data # 출력파일 == 입력피알 검증
...
경로를 출력해 서로 다른 테스트를 실행하거나 동일한 테스트를 두 번 실행하면 이를 확이할 수 있다.
...
```
- 해당 오퍼레이터는 operator.execute(context = {} )와 같이 dx변수를 사용할 수 없다.
